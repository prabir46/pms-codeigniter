...
; (function($) {
    ...
    $.fn.bcalendar = function(option) {
        ...
        function BuildDaysAndWeekView(startday, l, events, config) {
            ...
            var days = [];
            if (l == 1) {
                var show = dateFormat.call(startday, i18n.xgcalendar.dateformat.Md3);
                console.log(show);
                ...
                for (var i = w, j = 0; j < l; i = i + 1, j++) {
                    ...
                    var show = dateFormat.call(ndate, i18n.xgcalendar.dateformat.Md3);
            ...
            html.push("<div id=\"dvtec\"  class=\"scolltimeevent\"><table style=\"table-layout: fixed;","margin-top: -672px;", jQuery.browser.msie ? "" : "width:100%", "\" cellspacing=\"0\" cellpadding=\"0\"><tbody><tr><td>");
        ...
        function BuildWT(ht, dayarrs, events, dMax) {
            ...
            for (var i = 0; i < dayarrs.length; i++) {
                ...
                var dayOfWeek = new Date(dayarrs[i].date).getDay();
                var shortDayOfTheWeek = moment.weekdaysShort(dayOfWeek);
                ht.push("<th abbr='", dateFormat.call(dayarrs[i].date, i18n.xgcalendar.dateformat.fulldayvalue), "' class='gcweekname' scope=\"col\"><div title='", title, "' ", ev, " class='wk-dayname'><span class='", cl, "'>", dayarrs[i].display+' / '+ shortDayOfTheWeek, "</span></div></th>");
            }
        ...
        function BuildDayScollEventContainer(ht, dayarrs, events) {
            var countHours = ((60/option.intervalTime)/2)*24;
            //1:
            ht.push("<tr>");
            ...
            for (var i = 0; i < countHours; i++) {
                tmt = fomartTimeShow(i,option.intervalTime);
				
                ht.push("<div style=\"height: 41px\" class=\"tg-time\">", tmt, "</div>");
            }
            ht.push("</td>");
            var l = dayarrs.length;
            var height_col = countHours*42
            for (var i = 0; i < l; i++) {
                ...
                // Today
                if (istoday) {
                    ht.push("<div style=\"margin-bottom: -", height_col, "px; height:", height_col, "px\" class=\"tg-today\">&nbsp;</div>");
                }
                //var eventC = $(eventWrap);
                //onclick=\"javascript:FunProxy('rowhandler',event,this);\"
                ht.push("<div  style=\"margin-bottom: -", height_col, "px; height:", height_col, "px\" id='tgCol", i, "' class=\"tg-col-eventwrapper\">");
        ...
        function BuildEvents(hv, events, sday) {
            ...
            for (var i = 0; i < events.length; i++) {
                ...
                c=["#"+events[i].event[7],"#"+events[i].event[7],"#"+events[i].event[7],"#"+events[i].event[7]];
                
                var count;
                if(events[i].event[4]!=1 || events[i].event[5]!=1){
                    count = isOverlapped(events,i);
                    if(count==0){
                        events[i].aQ=0.9;
                        events[i].width=1;
                        events[i].left=0;
                        ...
                        count=count+1;
                        events[i].aQ=0.9/count;
                        events[i].width=1;
                        events[i].left=0.9/count;
                    }
   ...
   function isOverlapped(events,ci){
        ...
        function getTitle(event) {     
            ...
            var showtime = event[4] != 1;
             consultant_name = event[11]; 
             patient_name = event[1]; 
           patient_contact_no = event[12];
            eventshow = event[11];
            ...
            ret.push(i18n.xgcalendar.consultant_name + ": ", consultant_name , $.browser.mozilla?"":"\r\n", i18n.xgcalendar.patient_name + ": ", patient_name,$.browser.mozilla?"":"\r\n", i18n.xgcalendar.patient_contact_no + ": ", patient_contact_no);
            if (attendsshow != "") {
                ret.push($.browser.mozilla?"":"\r\n", i18n.xgcalendar.time + ": ", timeshow);
            }
            return ret.join("");
        }
...
//alert(p.title);
        ...
        function BuildMonthDayEvent(e, cday, length) {
            ...
            theme = ["#"+e.event[7],"#"+e.event[7],"#"+e.event[7],"#"+e.event[7]];
...
//alert(JSON.stringify(data));
        ...
        function parseDate(str){
            //var temp = new Date(Date.parse(str));
            //console.log(moment.weekdaysShort(temp.getDay()));
            return new Date(Date.parse(str));
        }
        function gP(h, m) {
            n = ((60/option.intervalTime)/2);
            return h * (42 * n) + parseInt(m / 60 * 42 * n);
        }
        function gW(ts1, ts2) {//edit event
            n = ((60/option.intervalTime)/2);
            var t1 = ts1 / (42 * n);
            var t2 = parseInt(t1);
            var t3 = 0;
            for(i=1;i<=(60/option.intervalTime);i++){
                if((t1 - t2).toFixed(10) >= ((option.intervalTime/60)*i).toFixed(10)){
                    t3 = option.intervalTime*i;
                }
            }
            var t4 = ts2 / (42 * n);
            var t5 = parseInt(t4);
            var t6 = 0;
            for(i=1;i<=(60/option.intervalTime);i++){
                if((t4 - t5).toFixed(10) >= ((option.intervalTime/60)*i).toFixed(10)){
                    t6 = option.intervalTime*i;
                }
            }
            return { sh: t2, sm: t3, eh: t5, em: t6, h: ts2 - ts1 };
        }
        function gH(y1, y2, pt) {//create new event
            n = ((60/option.intervalTime)/2);
        ...
        function tc(d) {
            function zc(c, i) {
                ...
                return "#" + d.substring(c * option.intervalTime + i * 6, c * option.intervalTime + (i + 1) * 6);
            }
        ...
        function fomartTimeShow(h,i) {
			
            m = Math.round(((h/((60/i)/2))*60)%60);
            h = Math.floor(h/((60/i)/2));
...
//alert(JSON.stringify(chip));
        ...
        function dayshow(e, data) {
            ...
            if (data != null) {
                if (option.quickDeleteUrl != "" && data[8] == 1 && option.readonly != true) {
                    ...
                    var csbuddle = '<div id="bbit-cs-buddle" style="z-index: 1803; width: 400px;visibility:hidden;" class="bubble"><table class="bubble-table" cellSpacing="0" cellPadding="0"><tbody><tr><td class="bubble-cell-side"><div id="tl1" class="bubble-corner"><div class="bubble-sprite bubble-tl"></div></div>'
                        +'<td class="bubble-cell-main"><div class="bubble-top"><h4 style="color: #454f9f;font-weight: bolder;">Appointment Details</h4></div><td class="bubble-cell-side"><div id="tr1" class="bubble-corner"><div class="bubble-sprite bubble-tr"></div></div>  <tr><td class="bubble-mid" colSpan="3"><div style="overflow: hidden" id="bubbleContent1"><div><div></div><div class="cb-root"><table class="cb-table" cellSpacing="0" cellPadding="0"><tbody><tr><td class="cb-value"><div class="textbox-fill-wrapper"><div class="textbox-fill-mid"><div id="bbit-cs-what" title="'
                        + i18n.xgcalendar.click_to_detail + '" class="textbox-fill-div " style="cursor:pointer;"></div></div></div></td></tr><tr><td class=cb-value><div id="bbit-cs-buddle-timeshow"></div></td></tr></tbody></table><div class="bbit-cs-split"><input id="bbit-cs-id" type="hidden" value=""/>  <span id="cancel_appointment" onclick="cancel_appointment_function();" class="lk">Cancel</span> &nbsp; [ <span id="bbit-cs-delete" class="lk">'
                        + i18n.xgcalendar.i_delete + '</span> ]&nbsp; <SPAN id="bbit-cs-editLink" class="lk">'
                        + i18n.xgcalendar.update_detail + ' </SPAN>&nbsp;&nbsp;<SPAN id="bbit-cs-addFollowLink" class="lk">'
                        + i18n.xgcalendar.add_follow_up + ' </SPAN></div></div></div></div><tr><td><div id="bl1" class="bubble-corner"><div class="bubble-sprite bubble-bl"></div></div><td><div class="bubble-bottom"></div><td><div id="br1" class="bubble-corner"><div class="bubble-sprite bubble-br"></div></div></tr></tbody></table><div id="bubbleClose2" class="bubble-closebutton"></div><div id="prong1" class="prong"><div class=bubble-sprite></div></div></div>';
                    ...
                    if (bud.length == 0) {
                        ...
                        var addfollowtn = $("#bbit-cs-addFollowLink");
                        ...
                        $("#bbit-cs-what").click(function(e) {
							...
							//console.log($("#bbit-cs-buddle").data("cdata"));
                            ...
                            if (!option.ViewCmdhandler) {
                                ...
                                if (option.ViewCmdhandler && $.isFunction(option.ViewCmdhandler)) {
									var detData = $("#bbit-cs-buddle").data("cdata");
									var paitentId = detData[9];
									//console.log('Petient IS : '+paitentId);
									//console.log('URL : '+redirectUrl);
									window.open(redirectUrl+'/'+paitentId,"_blank");
									return false;
						...
						addfollowtn.click(function(e) {
                            if (!option.AddFollowUpCmdhandler) {
                                alert("AddFollowUpCmdhandler" + i18n.xgcalendar.i_undefined);
                            }
                            else {
                                if (option.AddFollowUpCmdhandler && $.isFunction(option.AddFollowUpCmdhandler)) {
                                    option.AddFollowUpCmdhandler.call(this, $("#bbit-cs-buddle").data("cdata"));
                                }
                            }
                            $("#bbit-cs-buddle").css("visibility", "hidden");
                            return false;
					...
					console.log('Due : '+data[13]);
					
					var dueAmt = '';
					if(data[13] > 0)
						dueAmt = "<span class='col-md-5' style='color:#090;'>: "+data[13]+"</span>";
					else if(data[13] < 0)
						dueAmt = "<span class='col-md-5' style='color:#F00;'>: "+data[13]+"</span>";
					else
						dueAmt = "<span class='col-md-5'>: "+data[13]+"</span>";
					
                    var ts = $("#bbit-cs-buddle-timeshow").html(ss.join(""));
                    $("#bbit-cs-what").html("<div class='row'><span class='col-md-4'><b>Consultant Name</b> </span> <span class='col-md-5'>: "+data[11].toLowerCase().replace(/\b[a-z]/g, function(letter) { return letter.toUpperCase(); })+"</span></div><div class='row'><span class='col-md-4'><b>Patient Name</b></span> <span class='col-md-4'> : "+data[1].toUpperCase().replace(/\b[a-z]/g, function(letter) { return letter.toUpperCase(); })+"</span></div><div class='row'><span class='col-md-4'><b>Patient Contact No</b></span> <span class='col-md-4'> : "+cont.toLowerCase().replace(/\b[a-z]/g, function(letter) { return letter.toUpperCase(); })+"</span></div><div class='row'><span class='col-md-4'><b>Reason</b> </span> <span class='col-md-5'>: "+motive.toLowerCase().replace(/\b[a-z]/g, function(letter) { return letter.toUpperCase(); })+"</span></div><div class='row'><span class='col-md-4'><b>Due Amount</b> </span> "+dueAmt+"</div>"); 
        ...
        function dayupdate(data, start, end) {
            ...
            if (buddle.length == 0) {
                ...
                var temparr = [];
                temparr.push('<div id="bbit-cal-buddle" style="z-index: 180; width: 400px;visibility:hidden; display:none;" class="bubble">');
