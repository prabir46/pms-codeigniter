...
class dashboard_model extends CI_Model {
    ...
    function get_date_range_appointments($from_date,$consultant) {
        //echo $consultant;die('--');     
        $admin = $this->session->userdata('admin');
        if ($admin['user_role'] == 1) {
            $admin_id = $admin['id'];
        ...
        if ($consultant!= '' && $consultant!= NULL)
            $this->db->where('A.consultant', $consultant);
        
        $this->db->where('A.doctor_id', $admin_id);
        $this->db->select('A.*,U.name,C.name contact');
        $this->db->join('users U', 'U.id = A.patient_id', 'LEFT');
        $this->db->join('contacts  C', 'C.id = A.contact_id', 'LEFT');
        $data = $this->db->get('appointments A')->result();
        foreach ($data as $key => $value) {
            $data[$key]->date = date('d-m-Y h:i:s-a',strtotime($value->date));
        }
