...
class notification_model extends CI_Model {
    ...
    function get_invoice_header() {
        ...
        return $this->db->get('invoice_template')->row();
    }

    function update_invoice_header($save) {
        $data = $this->get_invoice_header();
        if (empty($data)) {

            $this->db->insert('invoice_template', $save);
        } else {
            $admin = $this->session->userdata('admin');
            $this->db->where('doctor_id', $admin['id']);
            $this->db->update('invoice_template', $save);
        }
    }
