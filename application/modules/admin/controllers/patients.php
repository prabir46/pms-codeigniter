...
class patients extends MX_Controller {
    ...
    public function __construct() {
        ...
        $this->load->model('patient_datatable_model', 'patients');
        //$this->load->model("treatment_advised_model");
        ...
        $no = $_POST['start'];
            foreach ($list as $patients) {
              ...
              if (!empty($patients)) {
                $row = array();
                $row[] = $no;
                $row[] = "<a href='" . site_url("admin/patients/view/" . $patients->id) . "' class='btn btn-link'>" . $patients->name . "</a>";
                $row[] = $patients->address;
                $row[] = $patients->gender;
                if($patients->dob ==0)
                {
                    $row[] = ''; 
                ...
                $row[] = date('Y') - $patients->dob;
                }
                $row[] = $patients->contact;
                $row[] = $patients->email;
                $row[] = $patients->username;
    ...
    function view($id = false, $tab = false) {
        ...
        $data['drug_Allergy'] = $this->drug_allergy_model->get_case_history_by_doctor();
         $data['medicine_template'] = $this->drug_allergy_model->get_case_history_by_doctor1();
    ...
    function reports($pre_id) {
        ...
        $data['prescriptions'] = $this->patient_model->get_patients_by_medication($pre_id);
   ...
   function add() {
        ...
        $admin = $this->session->userdata('admin');
          if($admin['user_role']==3){
          $ashu=$admin['doctor_id'];
          }
          else
          $ashu=$admin['id'];
        $this->db->where('doctor_id',$ashu);
        $settin=$this->db->get('settings')->row();
        $code=@$settin->username;
        $data['setting'] = $this->setting_model->get_setting();
            ...
            if ($this->form_validation->run() == true) {
                ...
                if ($admin['user_role'] == 1) {
                    ...
                    if (empty($username)) {
                        ...
                        $data['username'] = $code . ($sub_str + 1);
                        if(strlen($data['username'])>8)
                        {
                        $s=substr($data['username'],0,6);
                         $data['username'] = $s . ($sub_str + 1);
                         $data2['username']=$s;
                ...
                $save['name'] = strtoupper($this->input->post('name'));
    ...
    function search_patient() {		
		...
		if(count($result) > 0)
		{
			...
			foreach ($result as $resultpatient) {
				...
				{
				?>        
				<li><a href="<?php echo site_url('admin/patients/view/').'/'.$resultpatient->id; ?>"><?php echo $resultpatient->name; ?></a></li>
				<?php
				}
			}
   ...
   function add_patient(){
    ...
    function addNote(){
        $trtId=$this->input->post('treatId');
        $note=$this->input->post('note');
       print_r($note);
     
        $admin = $this->session->userdata('admin');
                $p_key = $this->patient_model->add_note($trtId, $note);
        ...
        function addPaymentNote(){
           ...
           print_r($note);
             ...
             $admin = $this->session->userdata('admin');
             $p_key = $this->patient_model->add_payment_note($trtId, $note);
             return $p_key;
                
            }
	
	function edit_treatment_plan() {
		$id=$this->input->post('id');
        $data['tooth_list'] = $this->prescription_model->edit_tooth($id);
		$data['treatment_Advised'] = $this->treatment_advised_model->get_case_history_by_doctor();
        $this->load->view('patients/edit_treatment_plan', $data);		
    }
	...
	function send_header_sms() {
		$this->load->model("sms_management_model");
		$sms_mobile_number=trim($this->input->post('sms_mobile_number'));
		$sms_mobile_message=trim($this->input->post('sms_mobile_message'));
		$sendMap=trim($this->input->post('sendMap'));
		
		$mapLink = '';
		if($sendMap == 1)
		{
			$settings = $this->setting_model->get_setting();
			$mapLink = @$settings->map_location;
		}
		
		$adminSee = $this->session->userdata('admin');
		if($adminSee['user_role']==1){
			$doctor_id = $adminSee['id'];
		}else{
			$doctor_id = $adminSee['doctor_id'];
		}
		...
		{
			if (!$this->validate_mobile($sms_mobile_number)) {
				echo json_encode(array("status"=>"error","message"=>'Invalid mobile number'));die;
			}
			else {
				...
				$senderId = "DCTRIP";
				$route = 4;
				$mobileNumber = $sms_mobile_number;
				//$mobileNumber = 9860130198;
				$encodedMessage = urlencode($sms_mobile_message.' '.$mapLink);
				...
				#Send sms again - ends  
				echo json_encode(array("status"=>"success","message"=>'SMS send successfully.'));die;
			}
		}	
		else
		{
			echo json_encode(array("status"=>"error","message"=>'Sorry, you does not have SMS credit left.'));die;	
		}
    }
	
	function appoint_cancel() {
		$this->load->model("sms_management_model");
		$appointment_cancel_id=trim($this->input->post('appointment_cancel_id'));
		$cancelBy=trim($this->input->post('cancelBy'));
		
		$query1 = "UPDATE `appointments` SET `Color` = 'F00', `motive` = 'Cancel by ".ucfirst($cancelBy)."' WHERE `id` = '".$appointment_cancel_id."'; ";
        $query = $this->db->query($query1);
		echo json_encode(array("status"=>"success","message"=>'Appointments cancelled successfully.'));die;
    }
}
