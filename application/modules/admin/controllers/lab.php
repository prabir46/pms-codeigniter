...
class lab extends MX_Controller {
	...
	function index(){
		...
		if($admin['user_role']=="Admin"){
			$data['case_historys'] = $this->lab_model->get_all();
		}
		if($admin['user_role']==1 || $admin['user_role']==3){
			$data['case_historys'] = $this->lab_model->get_case_history_by_doctor();
		}	
		$data['page_title'] = lang('lab');
		$data['body'] = 'lab/list';
	...
	function add(){
        ...
        {	
			...
			if ($this->form_validation->run()==true)
            {
				...
				$this->lab_model->save($save);
                $this->session->set_flashdata('message', "Lab Saved");
			echo 1;
			}else{
	...
	function edit($id=false){
		...
		$data['case_history'] = $this->lab_model->get_case_history_by_id($id);
		$admin = $this->session->userdata('admin');
		$data['id'] =$id;
