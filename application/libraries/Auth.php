...
class Auth
{
    ...
    */
    function login_admin($username, $password, $remember=false)
    {
        ...
        if (sizeof($result) > 0)
        {
            ...
            $admin['admin']['doctor_id'] = $result['id'];
            $admin['admin']['folder'] = base_url('');
            if($remember)
            {
			...
			//echo '2222 => ';
			#Added by pavan for doctor login - starts
			$this->CI->db->select('*');
            $this->CI->db->where('username', $username);
            $this->CI->db->where('password',  sha1($password));
            $this->CI->db->limit(1);
            ...
            if (sizeof($result) > 0)
            {
                ...
                $admin['admin']['user_role'] = $result['user_role'];
                $admin['admin']['image'] = $result['image'];
                $admin['admin']['doctor_id'] = $result['doctor_id'];
                $admin['admin']['folder'] = base_url('');
                if($remember)
                {
                    $loginCred = json_encode(array('username'=>$username, 'password'=>$password));
                ...
                $this->CI->session->set_userdata($admin);
                return true;
            }#Added by pavan for doctor login - ends
			else
			{
				//echo '4444 => ';die;
				$this->CI->db->select('*');
				...
				$this->CI->db->where('password',  sha1($password));
				$this->CI->db->limit(1);
				$result = $this->CI->db->get('assistant');
				$result = $result->row_array();
				if (sizeof($result) > 0)
				{
					$admin = array();
    ...
    */
    function save($admin)
    {
        ...
        {
            $this->CI->db->where('id', $admin['id']);
            $this->CI->db->update('doctor', $admin);
        }
        else
        {
            $this->CI->db->insert('doctor', $admin);
        }
    }
